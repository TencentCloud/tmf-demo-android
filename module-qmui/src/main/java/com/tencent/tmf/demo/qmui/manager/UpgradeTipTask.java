...
public class UpgradeTipTask implements UpgradeTask {
    ...
    public void upgrade(Activity activity) {
        String title = String
                .format(activity.getString(R.string.app_upgrade_tip_title), QMUIPackageHelper.getAppVersion(activity));
    ...
    private void appendBlockSpace(Context context, SpannableStringBuilder builder) {
        ...
        QMUIBlockSpaceSpan blockSpaceSpan = new QMUIBlockSpaceSpan(context, QMUIDisplayHelper.dp2px(context, 6));
    ...
    public CharSequence getUpgradeWord(final Activity activity) {
        ...
        if (mNewVersion == QDUpgradeManager.VERSION_1_4_0) {
            ...
            text.append(
                    "1. Updated arch library to 0.3.0. Now developer must update support library to 28 or use "
                            + "androidx.\n");
            ...
            text.append(
                    "4. Feature: allows control of the size of left icon in QMUIGroupListView.Section by calling the "
                            + "method setLeftIconSize.\n");
            ...
            appendBlockSpace(activity, text);
            text.append(
                    "4. The method startFragmentAndDestroyCurrent in QMUIFragment supports transfer of target "
                            + "fragment.\n");
            appendBlockSpace(activity, text);
    ...
    private void addNewWidget(final Activity activity, SpannableStringBuilder text, final String widgetName,
            final String docUrl) {
