...
public class LaunchInterceptor implements Interceptor {
    ...
    public void intercept(Chain chain) {
        new Handler(Looper.getMainLooper()).post(
                () -> new AlertDialog.Builder(chain.request().context()).setTitle(chain.request().context().getString(R.string.module_portal_6))
                        ...
                        .setNegativeButton(android.R.string.no,
                                (dialog, which) -> chain.terminate(
                                        Response.create(Response.STATUS_INTERCEPT).setMessage(
